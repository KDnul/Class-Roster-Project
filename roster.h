#pragma once
#ifndef ROSTER_H // Header Guard
#define ROSTER_H // Header Guard
#include<iostream>
...
class Roster {
	private:
		vector<Student*> classRosterArray; // Use vectors because it is safer. Can check for invalid indexes 
	...
	public:
		...
		void setClassRosterArray(vector<Student*> classRosterArray);

		// Get the Class Roster array
		vector<Student*> getClassRosterArray();

		// Functions
		void parse(string strRow);
		void add(string studentID, string firstName, string lastName, string emailAddress, int age, int daysInCourse1, int daysInCourse2, int daysInCourse3, Degree_Program degreeProgram);
		void remove(string studentID); 
		void printAll();
		void printAverageDaysInCourse(string studentID); 
		void printInvalidEmails(); // Emails can not contain spaces and must have '@' symbol and '.' period //done
		void printByDegreeProgram(Degree_Program degreeProgram);
};
